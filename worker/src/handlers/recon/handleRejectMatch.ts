...
export default async function handleRejectMatch(
  request: Request,
  env: Env,
  matchId: number,
): Promise<Response> {
  ...
  if (match.matched_by === session.userId) {
    return Response.json(
      { error: "Cannot reject your own proposed match" },
      { status: 403 },
    );
  }
  ...
  await env.recon_demo_db
    .prepare(
      "UPDATE MatchResults SET status = 'rejected', approved_by = ?, approved_at = datetime('now') WHERE id = ?",
    )
  ...
  await env.recon_demo_db
    .prepare(
      "INSERT INTO AuditLog (user_id, action, target_table, target_id, details) VALUES (?, 'reject_match', 'MatchResults', ?, NULL)",
    )
    .bind(session.userId, matchId)
    .run();

  return Response.json({ id: matchId, status: "rejected" });
}
